# main.py — Bot with WebApp form + admin approval
...
from aiogram.client.session.aiohttp import AiohttpSession  # proxy session
...
# helper: common flow for name submissions
...
# keyboards
btns = [[KeyboardButton(text="📝 فرم ساده")]]
if WEBAPP_URL:
    btns[0].append(KeyboardButton(text="🌐 فرم زیبا", web_app=WebAppInfo(url=WEBAPP_URL)))
start_kb = ReplyKeyboardMarkup(keyboard=btns, resize_keyboard=True)

# /start
@router.message(CommandStart())
async def on_start(message: types.Message):
    await message.answer("سلام! برای ارسال فرم یکی از گزینه‌ها را بزنید.", reply_markup=start_kb)

# simple form (text)
@router.message(F.text == "📝 فرم ساده")
async def open_form(message: types.Message, state: FSMContext):
    await state.set_state(Form.name)
    await message.answer("لطفاً اسم خود را بنویسید:", reply_markup=types.ReplyKeyboardRemove())
...
async def reject_callback(call: types.CallbackQuery):
    ...
    try:
        await bot.send_message(info["user_id"], "❌ فرم شما رد شد.")
    except Exception:
        pass
    await call.message.edit_text(call.message.text + "\n\n❌ رد شد.")
...
async def main():
    if not BOT_TOKEN:
        raise RuntimeError("BOT_TOKEN در .env تنظیم نشده.")
