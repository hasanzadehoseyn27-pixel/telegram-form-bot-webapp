...
from ..storage import is_admin, is_channel_allowed
...
async def cb_edit_price(call: types.CallbackQuery):
    ...
    await call.message.reply("قیمت جدید را با ارقام لاتین بفرستید (میلیون با اعشار یک‌رقمی یا تومانِ خالی).")
    await call.answer()
...
async def cb_publish(call: types.CallbackQuery):
    ...
    if not is_channel_allowed(SETTINGS.TARGET_GROUP_ID):
        await call.answer("⛔ دسترسی به کانال اصلی محدود است.", show_alert=True)
        return
    ...
    caption = build_caption(form, grp["number"], grp["jdate"], show_price=show_price, show_desc=show_desc)
    ...
    try:
        ...
        # اگر ادیت نشد، یک پیام تازه می‌فرستیم
        try:
