...
async def on_start(message: types.Message):
    ...
    if not await _user_is_member(message.bot, message.from_user.id):
        ...
        await message.answer("برای استفاده از ربات، ابتدا عضو کانال زیر شوید و سپس دوباره /start را بزنید:", reply_markup=join_kb)
        return
...
async def on_admin_text_edit(message: types.Message):
    ...
    if field == "price":
        ...
        if not ok: await message.reply("عدد نامعتبر. فقط ارقام لاتین؛ میلیون با اعشار یک‌رقمی (مثل 50.5) یا تومان خالی."); return
        form["price_num"] = n_toman; form["price_words"] = price_words(n_toman)
        await message.reply(f"قیمت به «{form['price_words']}» تغییر کرد.")
    elif field == "desc":
        form["desc"] = message.text.strip(); await message.reply("توضیحات به‌روزرسانی شد.")

    ADMIN_EDIT_WAIT.pop(message.from_user.id, None)
    await message.answer(admin_panel_text(form), parse_mode="HTML", reply_markup=admin_review_kb(token))
