...
# حافظه‌ی موقت
PENDING: dict[str, dict] = {}           # token -> {form, user_id, grp, needs, admin_msgs}
...
async def cb_edit_price(call: types.CallbackQuery):
    ...
    if token not in PENDING:
        await call.answer("درخواست یافت نشد/جلسه منقضی شده.", show_alert=True); return
...
# هندلر عمومی متن: اول ویرایش ادمین، بعد جریان لینک کانال
...
async def on_admin_text_edit(message: types.Message):
    ...
    if st:
        ...
        if slug.startswith("+") or slug.startswith("joinchat/") or slug.startswith("c/") or not re.fullmatch(r"[A-Za-z0-9_]{5,}", slug):
            await message.reply("❗ فقط لینک عمومی t.me/username پشتیبانی می‌شود.")
            return
        ref = "@" + slug if not slug.startswith("@") else slug
...
# ====== مدیریت کانال‌های مجاز (OWNER) – شروع/لیست ======
...
async def on_webapp_data(message: types.Message):
    # گیت عضویت: بدون عضویت هیچ داده‌ای پذیرفته نمی‌شود
    if not await _user_is_member(message.bot, message.from_user.id):
        await message.answer("⛔ ابتدا عضو کانال شوید و سپس دوباره تلاش کنید:", reply_markup=_join_kb())
        return
...
async def on_photo(message: types.Message):
    # گیت عضویت روی ارسال عکس نیز اعمال می‌شود
    if not await _user_is_member(message.bot, message.from_user.id):
        await message.reply("⛔ ابتدا عضو کانال شوید:", reply_markup=_join_kb())
        return
...
async def cb_finish(call: types.CallbackQuery):
    ...
    # گیت عضویت هنگام انتشار (اگر بعداً لفت داده باشد، جلوگیری می‌شود)
    if not await _user_is_member(call.bot, call.from_user.id):
        await call.answer("⛔ ابتدا عضو کانال شوید و سپس دوباره تلاش کنید.", show_alert=True)
        try:
            await call.message.answer("برای عضویت:", reply_markup=_join_kb())
        except Exception:
            pass
        return
